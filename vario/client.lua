...
Citizen.CreateThread(function()
    while true do
       Citizen.Wait(0) -- Prevenzione crash
...
function Display(coordsPlayer, idPlayer, identifierPlayer, reasonPlayer)
    ...
    local testo = 'Uscito'
    reasonPlayer = string.lower(tostring(reasonPlayer))
    if string.match(reasonPlayer, 'crashed') or string.match(reasonPlayer, 'timed') or string.match(reasonPlayer, 'stopped') or string.match(reasonPlayer, 'funzionare') or string.match(reasonPlayer, 'working') then
        testo = 'Crashato'
    elseif string.match(reasonPlayer, 'bannato') then
        testo = 'Bannato'
    elseif string.match(reasonPlayer, 'clown') or string.match(reasonPlayer, 'devtool') then
        testo = 'Bannato [AntiCheat]'
    elseif string.match(reasonPlayer, 'respawned') then
        testo = 'Respawnato'
    end
    Citizen.CreateThread(function()
        Wait(50000)
    ...
    Citizen.CreateThread(function()
        while displaying do
            local CicloExit = 2500
            local distanza = #(GetEntityCoords(PlayerPedId()) - coordsPlayer)
            if distanza < 30.0 then
                CicloExit = 5
                DrawText3D(coordsPlayer, '~g~Giocatore '..testo..'~w~ \nID: '..idPlayer..' ('..identifierPlayer..')')
            end
            Citizen.Wait(CicloExit)
        end
    end)
end

function DrawText3D(value, text)
    local onScreen,_x,_y=World3dToScreen2d(value.x,value.y,value.z)
    ...
    if args[1] then
        ...
        if metaCercata then
            ...
            ESX.ShowNotification = function(msg, flash, saveToBrief, hudColorIndex)
                TriggerEvent('pnpNotify:send', '<b><span style="color:#33cc33;">NOTIFICA</span></b> </br>'..msg)
        else
            ESX.ShowNotification = function(msg, flash, saveToBrief, hudColorIndex)
                TriggerEvent('pnpNotify:send', '<b><span style="color:#33cc33;">NOTIFICA</span></b> </br>'..msg)
        end
